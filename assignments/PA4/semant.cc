...
static std::unordered_map<Symbol, MethodTable> methodtables;
...
ClassTable::ClassTable(Classes classes) : semant_errors(0) , error_stream(cerr) {
    ...
    // std::unordered_map<Symbol, Class_> ClassTable::m_classes
    // ==============================================
    ...
    for (int i = classes->first(); classes->more(i); i = classes->next(i)) {
        ...
        if (!m_classes.count(classes->nth(i)->GetName() )) {
            m_classes[classes->nth(i)->GetName()] = classes->nth(i);
        } else {
    ...
    for (int i = classes->first(); classes->more(i); i = classes->next(i)) {
        ...
        while (parent_name != Object && parent_name != classes->nth(i)->GetName()) {
            ...
            if (!m_classes.count(parent_name)) {
...
void method_class::CheckFeatureType() {
    ...
    for (int i = formals->first(); formals->more(i); i = formals->next(i)) {
        ...
        if (!classtable->m_classes.count(type) ) {
...
Symbol new__class::CheckExprType() {
    if (type_name != SELF_TYPE && !classtable->m_classes.count(type_name) ) {
...
void program_class::semant() {
    ...
    // std::unordered_map<Symbol, Class_> m_classes;
    for (std::unordered_map<Symbol, Class_>::iterator iter = classtable->m_classes.begin(); iter != classtable->m_classes.end(); ++iter) {
